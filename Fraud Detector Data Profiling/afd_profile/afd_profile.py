...
def get_overview(config, df):
    ...
    try:
        df['event_timestamp'] = pd.to_datetime(df[config['required_features']['EVENT_TIMESTAMP']])
...
def set_feature(row, config):
    ...
    # -- null pct --   
    if (row.null_pct >= 0.75 and (rulehit == 0)):
        rulehit = 1
        feature = "exclude"
    ...
    if row._column == required_features['FRAUD_LABEL']:
        feature = "FRAUD_LABEL"
...
def get_label(config, df):
    ...
    message = {}
    label = config['required_features']['FRAUD_LABEL']
...
def get_partition(config, df):
    ...
    label_summary = df[required_features['FRAUD_LABEL']].value_counts()
    ...
    ctab = pd.crosstab(df['_dt'].astype(str), df[required_features['FRAUD_LABEL']]).reset_index()
    ...
    # -- partition by fraud label cross tab 
    rec = pd.crosstab(df['partition'],df[required_features['FRAUD_LABEL']]).reset_index()
    eval_count = rec.loc[rec['partition'] == 'evaluation'][1].to_numpy()
    test_count = rec.loc[rec['partition'] == 'testing'][1].to_numpy()
    
    # -- partition checks -- 
    if eval_count <= 75:
        message['eval < 75'] =  str(eval_count) + " fraud records in evaluation set, a minimum of 75 records required"
    
    if test_count <= 75:
        message['test_75'] = str(test_count) + " fraud records in testing set, a minimum of 75 records required"
        
    if (rec[1].min()/row_count)/(rec[1].max()/row_count) <= 0.2 :
        message['min/max < 0.2']  = "Inconsistent fraud rate accross partitions"
...
def get_email(config, df):
    ...
    ctab = pd.crosstab(df['domain'], df[required_features['FRAUD_LABEL']],).reset_index()
...
def get_ip_address(config, df):
    ...
    ctab = pd.crosstab(df[required_features['IP_ADDRESS']], df[required_features['FRAUD_LABEL']],).reset_index()
...
def get_categorical(config, df_stats, df):
    ...
    target = required_features['FRAUD_LABEL']
